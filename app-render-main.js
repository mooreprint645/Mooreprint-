...
function renderAll() {
  ...
  renderReports();
  fillBusinessForm();
  window.MoorePrintMonthlyCosts?.render?.();
  window.MoorePrintBusinessAssistant?.render?.();
}

function accountingExpenseSummary(expenses, orders) {
  const total = sum(expenses, expense => Math.max(0, num(expense.amount)));
  const marked = sum(expenses.filter(expense => expense.includedInPricing), expense => Math.max(0, num(expense.amount)));
  const embedded = sum(orders, order => documentTotals(order).allocatedOverhead);
  const reconciled = Math.min(marked, embedded);
  return { total, marked, embedded, reconciled, operating: Math.max(0, total - reconciled) };
}
...
function renderDashboard() {
  ...
  $('#metricOrders').textContent = `${data.orders.length} pedido${data.orders.length === 1 ? '' : 's'} · IVA ${money(data.taxes)}`;
  ...
  const chartData = [
    { label: 'Ventas netas', value: data.sales, className: '' },
  ...
  $('#dashboardAlerts').innerHTML = alerts.length
    ? alerts.map(alert => `<button class="alert-row text-button" data-go="${alert.section}"><span class="alert-icon">${alert.icon}</span><span class="alert-content"><strong>${esc(alert.title)}</strong><small>${esc(alert.detail)}</small></span><span>›</span></button>`).join('')
    : '<p class="empty-message">Todo está al corriente.</p>';

  const upcoming = state.orders
    .filter(order => !['entregado', 'cancelado'].includes(order.status))
    .sort((a, b) => String(a.dueDate).localeCompare(String(b.dueDate)))
    .slice(0, 6);
  $('#upcomingOrders').innerHTML = upcoming.length
    ? upcoming.map(order => `<div class="mini-row"><div><strong>${esc(order.folio)} · ${esc(order.customer || entityName(state.customers, order.customerId))}</strong><small>${formatDate(order.dueDate)} · ${statusName(order.status)}</small></div><span class="${isOverdue(order) ? 'money-negative' : ''}">${money(documentTotals(order).total)}</span></div>`).join('')
    : '<p class="empty-message">No hay entregas pendientes.</p>';

  const pending = state.orders
    .filter(order => order.status !== 'cancelado' && documentTotals(order).balance > 0)
    .sort((a, b) => documentTotals(b).balance - documentTotals(a).balance)
    .slice(0, 6);
  $('#pendingPayments').innerHTML = pending.length
    ? pending.map(order => `<div class="mini-row"><div><strong>${esc(order.customer || entityName(state.customers, order.customerId))}</strong><small>${esc(order.folio)}</small></div><span class="money-warning">${money(documentTotals(order).balance)}</span></div>`).join('')
    : '<p class="empty-message">No hay cobros pendientes.</p>';
}

function balanceDisplay(totals) {
  return totals.credit > 0
    ? { text: `A favor ${money(totals.credit)}`, className: 'money-positive' }
    : { text: money(totals.balance), className: totals.balance > 0 ? 'money-warning' : 'money-positive' };
}
...
function renderOrders() {
  ...
  const orders = [...state.orders]
    ...
    .filter(order => `${order.folio} ${order.customer} ${entityName(state.customers, order.customerId, '')} ${(order.items || []).map(item => item.name).join(' ')}`.toLowerCase().includes(query))
  ...
  $('#ordersTable').innerHTML = orders.map(order => {
    ...
    return `<tr class="${isOverdue(order) ? 'stock-low' : ''}"><td><strong>${esc(order.folio)}</strong><br><small>${esc(order.priority || 'normal')}</small></td><td><strong>${esc(order.customer || entityName(state.customers, order.customerId))}</strong><br><small>${esc(order.phone || '')}</small></td><td title="${esc(description)}">${esc(description.slice(0, 58))}${description.length > 58 ? '…' : ''}<br><small>${esc(order.designStatus || 'pendiente')} · ${esc(order.responsible || 'Sin responsable')}</small></td><td>${formatDate(order.dueDate)}${isOverdue(order) ? '<br><small class="money-negative">Atrasado</small>' : ''}</td><td><span class="badge ${order.status}">${statusName(order.status)}</span></td><td>${money(totals.total)}</td><td class="${balance.className}">${balance.text}</td><td><div class="action-group"><button class="action-button" data-view-order="${order.id}" title="Nota">🧾</button><button class="action-button" data-pay-order="${order.id}" title="Registrar pago">$</button><button class="action-button" data-edit-order="${order.id}" title="Editar">✎</button><button class="action-button" data-delete-order="${order.id}" title="Eliminar">×</button></div></td></tr>`;
  }).join('');
}
...
function renderQuotes() {
  ...
  state.quotes.forEach(quote => {
    if (!['aceptada', 'rechazada', 'convertida'].includes(quote.status) && quote.validUntil && quote.validUntil < todayISO()) quote.status = 'vencida';
  });
  const quotes = [...state.quotes]
    .filter(quote => filter === 'all' || quote.status === filter)
    .filter(quote => `${quote.folio} ${quote.customer} ${entityName(state.customers, quote.customerId, '')}`.toLowerCase().includes(query))
...
function customerStats(customerId) {
  ...
  return {
    ...
    sales: sum(orders, order => documentTotals(order).netRevenue),
    balance: sum(orders, order => documentTotals(order).balance)
  };
}
...
function renderCustomers() {
  ...
  $('#customersGrid').innerHTML = customers.map(customer => {
    ...
    return `<article class="entity-card"><div class="entity-card-header"><div><h3>${esc(customer.name)}</h3><p>${esc(customer.phone || 'Sin teléfono')}</p><p>${esc(customer.email || '')}</p></div><div class="action-group"><button class="action-button" data-view-customer="${customer.id}">◉</button><button class="action-button" data-edit-customer="${customer.id}">✎</button><button class="action-button" data-delete-customer="${customer.id}">×</button></div></div><p>${esc(customer.address || '')}</p><p>RFC: ${esc(customer.rfc || '—')}</p><div class="entity-stats"><div class="entity-stat"><span>Pedidos</span><strong>${stats.orders}</strong></div><div class="entity-stat"><span>Ventas netas</span><strong>${money(stats.sales)}</strong></div><div class="entity-stat"><span>Debe</span><strong class="${stats.balance ? 'money-warning' : ''}">${money(stats.balance)}</strong></div></div></article>`;
  }).join('');
}
...
function renderProducts() {
  ...
  $('#productGrid').innerHTML = products.map(product => {
    const breakdown = productBreakdown(product);
    const margin = num(product.salePrice) ? breakdown.profit / num(product.salePrice) * 100 : 0;
    const recipeCount = (product.recipe || []).filter(row => row.materialId).length;
    return `<article class="product-card"><div class="product-card-header"><div><h3>${esc(product.name)}</h3><span class="category">${esc(product.category || 'Sin categoría')}</span></div><div class="action-group"><button class="action-button" data-edit-product="${product.id}">✎</button><button class="action-button" data-delete-product="${product.id}">×</button></div></div><div class="product-price">${money(product.salePrice)}</div><div class="cost-list"><div class="cost-line"><span>Materiales</span><strong>${money(breakdown.material)}</strong></div><div class="cost-line"><span>Proceso y servicios</span><strong>${money(breakdown.processBase)}</strong></div><div class="cost-line"><span>Costos fijos asignados</span><strong>${money(breakdown.overhead)}</strong></div><div class="cost-line"><span>Desperdicio y comisión</span><strong>${money(breakdown.waste + breakdown.commission)}</strong></div><div class="cost-line total"><span>Costo unitario</span><strong>${money(breakdown.total)}</strong></div></div><div class="product-footer"><strong class="${breakdown.profit < 0 ? 'money-negative' : 'money-positive'}">Ganas ${money(breakdown.profit)}</strong><span class="margin-pill ${margin < 20 ? 'low' : ''}">${margin.toFixed(1)}%</span></div><small>${recipeCount} material${recipeCount === 1 ? '' : 'es'} en receta</small></article>`;
  }).join('');
}
...
function renderInventory() {
  ...
  const materials = state.materials
    .filter(material => `${material.name} ${material.category} ${entityName(state.suppliers, material.supplierId, '')}`.toLowerCase().includes(query))
    .filter(material => filter === 'all' || (filter === 'low' && isLowStock(material)) || (filter === 'out' && num(material.stock) <= 0));
  ...
  const movements = [...state.inventoryMovements]
    ...
    .slice(0, 40);
  $('#inventoryMovementsTable').innerHTML = movements.length
    ? movements.map(move => `<tr><td>${formatDate(move.date)}</td><td>${esc(entityName(state.materials, move.materialId))}</td><td><span class="badge ${move.quantity >= 0 ? 'pagado' : 'vencido'}">${esc(move.type)}</span></td><td class="${move.quantity >= 0 ? 'money-positive' : 'money-negative'}">${move.quantity >= 0 ? '+' : ''}${num(move.quantity).toFixed(2)}</td><td>${esc(move.reason)}</td></tr>`).join('')
...
function supplierStats(supplierId) {
  ...
  return {
    ...
    total: sum(purchases, purchase => purchaseTotals(purchase).total),
    balance: sum(purchases, purchase => purchaseTotals(purchase).balance)
  };
}

function renderSuppliers() {
  const service = window.MoorePrintSupplierCatalog;
  if (service?.render) return service.render();
  $('#suppliersGrid').innerHTML = '<div class="empty-state">El módulo de proveedores no está disponible.</div>';
}
