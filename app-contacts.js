...
function openCustomerModal(id = '') {
  ...
  openModal(id ? 'Editar cliente' : 'Nuevo cliente', `<form id="customerForm" class="modal-form"><input type="hidden" name="id" value="${esc(customer.id || '')}"><label>Nombre comercial<input name="name" required value="${esc(customer.name || '')}"></label><label>Teléfono / WhatsApp<input name="phone" value="${esc(customer.phone || '')}"></label><label>Correo<input name="email" type="email" value="${esc(customer.email || '')}"></label><label>RFC<input name="rfc" maxlength="13" value="${esc(customer.rfc || '')}"></label><label class="full">Dirección<input name="address" value="${esc(customer.address || '')}"></label><div class="info-box full"><strong>Datos fiscales CFDI 4.0</strong><p>Son opcionales hasta que el cliente solicite factura. Deben coincidir con su constancia de situación fiscal.</p></div><label class="full">Nombre o razón social fiscal<input name="fiscalName" value="${esc(customer.fiscalName || '')}"></label><label>Código postal fiscal<input name="fiscalPostalCode" inputmode="numeric" maxlength="5" pattern="[0-9]{5}" value="${esc(customer.fiscalPostalCode || '')}"></label><label>Régimen fiscal (clave)<input name="fiscalRegime" list="fiscalRegimeOptions" maxlength="3" value="${esc(customer.fiscalRegime || '')}"></label><label>Uso CFDI habitual<input name="cfdiUse" list="cfdiUseOptions" maxlength="4" value="${esc(customer.cfdiUse || '')}"></label><label>Correo para factura<input name="invoiceEmail" type="email" value="${esc(customer.invoiceEmail || customer.email || '')}"></label><label class="full">Notas<textarea name="notes" rows="3">${esc(customer.notes || '')}</textarea></label><datalist id="fiscalRegimeOptions"><option value="601">General de Ley Personas Morales</option><option value="603">Personas Morales con Fines no Lucrativos</option><option value="605">Sueldos y Salarios</option><option value="606">Arrendamiento</option><option value="612">Personas Físicas con Actividades Empresariales y Profesionales</option><option value="616">Sin obligaciones fiscales</option><option value="621">Incorporación Fiscal</option><option value="625">Plataformas Tecnológicas</option><option value="626">Régimen Simplificado de Confianza</option></datalist><datalist id="cfdiUseOptions"><option value="G01">Adquisición de mercancías</option><option value="G02">Devoluciones, descuentos o bonificaciones</option><option value="G03">Gastos en general</option><option value="I01">Construcciones</option><option value="I04">Equipo de cómputo y accesorios</option><option value="S01">Sin efectos fiscales</option><option value="CP01">Pagos</option></datalist></form>`, `<button class="button secondary" data-close-modal>Cancelar</button><button class="button primary" form="customerForm">Guardar cliente</button>`, true);
}
