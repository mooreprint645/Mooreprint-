...
function categoryName(category) {
  const names = {
    materiales: 'Materiales', pasajes: 'Pasajes', gasolina: 'Gasolina', gas: 'Gas',
    renta_local: 'Renta o local', servicios: 'Servicios', mantenimiento: 'Mantenimiento', otros: 'Otros'
  };
  return names[category] || category || 'Otros';
}

function orderStatusName(status) {
  return ({ pendiente: 'Pendiente', en_proceso: 'En proceso', listo: 'Listo', entregado: 'Entregado', cancelado: 'Cancelado' })[status] || status;
}
...
function orderTotals(order) {
  ...
  return { sales, costs, profit: sales - costs, balance: Math.max(0, sales - number(order.paid)) };
}
...
function periodData(period = 'month') {
  ...
  const orders = state.orders.filter(order => isActiveOrder(order) && inRange(order.orderDate, range.from, range.to));
...
function renderDashboard() {
  ...
  const max = Math.max(...chartValues.map(item => item.value), 1);
  $('#summaryChart').innerHTML = chartValues.map(item => `
    <div class="bar-item">
      <strong>${money(item.value)}</strong>
      <div class="bar ${item.className}" style="height:${Math.max(4, (item.value / max) * 145)}px"></div>
  ...
  const pending = data.orders
    ...
    .slice(0, 5);
  $('#pendingPayments').innerHTML = pending.length ? pending.map(item => `
    <div class="mini-row"><div><strong>${escapeHtml(item.order.customer)}</strong><small>${escapeHtml(item.order.folio)}</small></div><span>${money(item.balance)}</span></div>`).join('')
  ...
  $('#recentOrdersTable').innerHTML = recent.length ? recent.map(order => {
    const totals = orderTotals(order);
    return `<tr>
      <td><strong>${escapeHtml(order.folio)}</strong></td><td>${escapeHtml(order.customer)}</td><td>${formatDate(order.orderDate)}</td>
      <td><span class="badge ${order.status}">${orderStatusName(order.status)}</span></td><td>${money(totals.sales)}</td>
  ...
  $('#productGrid').innerHTML = products.map(product => {
    ...
    return `<article class="product-card">
      <div class="product-card-header">
        ...
        <div class="action-group"><button class="action-button" data-edit-product="${product.id}" title="Editar">✎</button><button class="action-button" data-delete-product="${product.id}" title="Eliminar">×</button></div>
      </div>
      ...
      <div class="product-footer"><strong class="${profit < 0 ? 'money-negative' : 'money-positive'}">Ganas ${money(profit)}</strong><span class="margin-pill ${margin < 20 ? 'low' : ''}">${margin.toFixed(1)}%</span></div>
    </article>`;
  }).join('');
}
...
function renderOrders() {
  ...
  $('#ordersTable').innerHTML = orders.map(order => {
    ...
    return `<tr>
      ...
      <td>${money(totals.sales)}</td><td>${money(order.paid)}<br><small>Falta ${money(totals.balance)}</small></td>
...
function renderExpenses() {
  ...
  $('#expensesTable').innerHTML = expenses.map(expense => `<tr>
    <td>${formatDate(expense.date)}</td><td>${categoryName(expense.category)}</td><td>${escapeHtml(expense.description)}</td>
    <td class="money-negative">${money(expense.amount)}</td><td><div class="action-group"><button class="action-button" data-edit-expense="${expense.id}">✎</button><button class="action-button" data-delete-expense="${expense.id}">×</button></div></td>
  </tr>`).join('');
}
...
function renderReports() {
  ...
  const orders = state.orders.filter(order => isActiveOrder(order) && inRange(order.orderDate, from, to));
  ...
  const groupedExpenses = expenses.reduce((map, expense) => {
    map[expense.category] = (map[expense.category] || 0) + number(expense.amount);
    return map;
  }, {});
  const expenseRows = Object.entries(groupedExpenses).sort((a, b) => b[1] - a[1]);
  const maxExpense = Math.max(...expenseRows.map(([, value]) => value), 1);
  $('#expenseCategoryReport').innerHTML = expenseRows.length ? expenseRows.map(([key, value]) => `
    <div class="category-row"><span>${categoryName(key)}</span><div class="progress-track"><div class="progress-fill" style="width:${(value / maxExpense) * 100}%"></div></div><strong>${money(value)}</strong></div>`).join('')
    : '<p class="empty-message">Sin gastos en este periodo.</p>';

  const products = {};
  orders.forEach(order => (order.items || []).forEach(item => {
    const key = item.name || 'Sin nombre';
    products[key] = (products[key] || 0) + number(item.qty) * number(item.price);
  }));
  const top = Object.entries(products).sort((a, b) => b[1] - a[1]).slice(0, 8);
  $('#topProductsReport').innerHTML = top.length ? top.map(([name, value], index) => `<div class="mini-row"><div><strong>${index + 1}. ${escapeHtml(name)}</strong></div><span>${money(value)}</span></div>`).join('')
...
function modalShell(title, body, footer = '') {
  return `<div class="modal-header"><h2 id="modalTitle">${escapeHtml(title)}</h2><button class="modal-close" data-close-modal>×</button></div><div class="modal-body">${body}</div>${footer ? `<div class="modal-footer">${footer}</div>` : ''}`;
}
...
function openProductModal(productId = '') {
  ...
  openModal(modalShell(productId ? 'Editar producto' : 'Nuevo producto', `
    ...
    </form>`, `<button class="button secondary" data-close-modal>Cancelar</button><button class="button primary" type="submit" form="productForm">Guardar producto</button>`));
}

function saveProduct(form) {
  const data = Object.fromEntries(new FormData(form));
  const product = {
    id: data.id || uid('product'), name: data.name.trim(), category: data.category.trim(),
    salePrice: number(data.salePrice), materialCost: number(data.materialCost), laborCost: number(data.laborCost),
    extraCost: number(data.extraCost), notes: data.notes.trim(), updatedAt: new Date().toISOString()
  };
...
function lineTemplate(item = {}) {
  const productOptions = state.products.map(product => `<option value="${product.id}" ${item.productId === product.id ? 'selected' : ''}>${escapeHtml(product.name)}</option>`).join('');
  return `<div class="order-line">
    <select class="line-product" aria-label="Producto"><option value="">Concepto personalizado</option>${productOptions}</select>
...
function openOrderModal(orderId = '') {
  const order = state.orders.find(item => item.id === orderId) || {
    folio: nextFolio(), customer: '', phone: '', orderDate: todayISO(), dueDate: todayISO(), status: 'pendiente', paid: 0, notes: '', items: [{}]
  };
  openModal(modalShell(orderId ? `Editar pedido ${order.folio}` : 'Nuevo pedido', `
    <form id="orderForm" class="modal-form">
      <input type="hidden" name="id" value="${escapeHtml(order.id || '')}"><input type="hidden" name="folio" value="${escapeHtml(order.folio)}">
      ...
      <div class="line-items">
        <div class="line-items-header"><div><strong>Productos o trabajos</strong><br><small>Venta y costo son valores por unidad.</small></div><button type="button" class="button secondary" id="addOrderLine">+ Agregar</button></div>
    ...
    </form>`, `<button class="button secondary" data-close-modal>Cancelar</button><button class="button primary" type="submit" form="orderForm">Guardar pedido</button>`));
...
function saveOrder(form) {
  ...
  const items = $$('.order-line', form).map(line => {
    ...
    return {
      productId, name: customName || selected?.name || 'Trabajo sin nombre', qty: Math.max(1, number($('.line-qty', line).value)),
      price: number($('.line-price', line).value), cost: number($('.line-cost', line).value)
    };
  }).filter(item => item.name && item.qty > 0);
  if (!items.length) return showToast('Agrega al menos un producto al pedido', true);
  const order = {
    id: data.id || uid('order'), folio: data.folio || nextFolio(), customer: data.customer.trim(), phone: data.phone.trim(),
    orderDate: data.orderDate, dueDate: data.dueDate, status: data.status, paid: number(data.paid), notes: data.notes.trim(),
...
function openExpenseModal(expenseId = '') {
  const expense = state.expenses.find(item => item.id === expenseId) || { date: todayISO(), category: 'materiales', description: '', amount: 0 };
  openModal(modalShell(expenseId ? 'Editar gasto' : 'Nuevo gasto', `
    ...
    </form>`, `<button class="button secondary" data-close-modal>Cancelar</button><button class="button primary" type="submit" form="expenseForm">Guardar gasto</button>`));
}

function saveExpense(form) {
  const data = Object.fromEntries(new FormData(form));
  const expense = { id: data.id || uid('expense'), date: data.date, category: data.category, description: data.description.trim(), amount: number(data.amount), updatedAt: new Date().toISOString() };
...
function openOrderPreview(orderId) {
  ...
  openModal(modalShell(`Nota ${order.folio}`, `
    <div class="note-preview">
      <div class="note-business"><h2>${escapeHtml(business.name)}</h2><p>${escapeHtml(business.city || '')}</p><p>${escapeHtml(business.phone || '')}</p></div>
      <div class="note-meta"><div><strong>Folio:</strong> ${escapeHtml(order.folio)}</div><div><strong>Pedido:</strong> ${formatDate(order.orderDate)}</div><div><strong>Cliente:</strong> ${escapeHtml(order.customer)}</div><div><strong>Entrega:</strong> ${formatDate(order.dueDate)}</div></div>
      <table><thead><tr><th>Cant.</th><th>Descripción</th><th>Precio</th><th>Importe</th></tr></thead><tbody>
        ${(order.items || []).map(item => `<tr><td>${item.qty}</td><td>${escapeHtml(item.name)}</td><td>${money(item.price)}</td><td>${money(number(item.qty) * number(item.price))}</td></tr>`).join('')}
      </tbody></table>
      <div class="order-totals" style="margin-top:16px"><div class="total-row"><span>Total</span><strong>${money(totals.sales)}</strong></div><div class="total-row"><span>Pagado</span><strong>${money(order.paid)}</strong></div><div class="total-row net"><span>Restante</span><strong>${money(totals.balance)}</strong></div></div>
    ...
    </div>`, `<button class="button secondary" data-close-modal>Cerrar</button><button class="button primary" id="printOrderButton">Imprimir nota</button>`));
}

function confirmDelete(type, id) {
  const labels = { product: 'producto', order: 'pedido', expense: 'gasto' };
  openModal(modalShell(`Eliminar ${labels[type]}`, `<p>Esta acción no se puede deshacer. ¿Deseas continuar?</p>`, `<button class="button secondary" data-close-modal>Cancelar</button><button class="button danger" data-confirm-delete="${type}" data-id="${id}">Eliminar</button>`));
}
...
function loadExamples() {
  const examples = [
    ...
    { name: 'Etiqueta adhesiva', category: 'Etiquetas', salePrice: 6, materialCost: 1.5, laborCost: 1, extraCost: .5, notes: 'Costo estimado por pieza' }
  ];
  examples.forEach(example => {
    if (!state.products.some(product => product.name.toLowerCase() === example.name.toLowerCase())) {
      state.products.push({ ...example, id: uid('product'), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
    }
  });
...
function exportBackup() {
  ...
  downloadBlob(blob, `mooreprint-respaldo-${todayISO()}.json`);
...
function importBackup(file) {
  ...
  reader.onload = () => {
    try {
      ...
      if (!Array.isArray(imported.products) || !Array.isArray(imported.orders) || !Array.isArray(imported.expenses)) throw new Error();
      state = { products: imported.products, orders: imported.orders, expenses: imported.expenses, business: { ...defaultState.business, ...(imported.business || {}) } };
...
function exportCsv() {
  ...
  state.orders.filter(order => isActiveOrder(order) && inRange(order.orderDate, from, to)).forEach(order => {
    const totals = orderTotals(order);
  ...
  state.expenses.filter(expense => inRange(expense.date, from, to)).forEach(expense => {
    rows.push(['Gasto', expense.date, categoryName(expense.category), expense.description, 0, 0, expense.amount, -number(expense.amount)]);
  });
...
function setupEvents() {
  ...
  $('#importBackupInput').addEventListener('change', event => event.target.files[0] && importBackup(event.target.files[0]));
