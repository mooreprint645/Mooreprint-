...
const script = fs.readFileSync('purchase-packages-v2.js', 'utf8');

async function prepare(page, unit = 'pieza') {
  await page.setContent('<!doctype html><html><head></head><body></body></html>');
  await page.evaluate(selectedUnit => {
    window.state = {
      supplierCatalog: [{
        id: 'catalog-11oz',
...
test('corrige una unidad numérica dentro del mismo módulo de compras', async ({ page }) => {
  await prepare(page, '1');
  const result = await page.evaluate(() => {
    document.body.innerHTML = `<form id="purchaseForm">
      <select name="supplierId"><option value="supplier-1" selected>Proveedor</option></select>
      <div id="purchaseRows">${window.purchaseRow({
        materialId: 'material-11oz',
        catalogItemId: 'catalog-11oz',
        packageCount: 1,
        presentationQty: 36,
    ...
    return {
      ...
      option: row.querySelector('.purchase-presentation').selectedOptions[0].textContent,
      conversion: row.querySelector('.purchase-conversion').textContent,
      line: window.MoorePrintPurchasePackages.purchaseLineFromRow(row)
    };
  });

  expect(result.materialUnit).toBe('pieza');
  expect(result.catalogUnit).toBe('pieza');
  expect(result.option).toContain('caja de 36 pieza');
  expect(result.conversion).toContain('36 pieza al inventario');
  expect(result.conversion).toContain('$15.28 por pieza');
  expect(result.line.unit).toBe('pieza');
});
